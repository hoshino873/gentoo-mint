...
SRC_URI="${UURL}.orig.tar.gz ${UURL}-${UREV}.debian.tar.xz"
...
src_install() {
	...
	mv "${WORKDIR}"/debian/changelog{,.ubuntu} || die
	mv "${WORKDIR}"/debian/copyright{,.ubuntu} || die

	dodoc "${WORKDIR}"/debian/{changelog,copyright}.ubuntu || die
	dodoc README NEWS CHANGELOG COPYING* || die
